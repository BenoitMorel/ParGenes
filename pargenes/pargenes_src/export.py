...
def get_input_parameters():
    ...
    parser.add_argument("--gene-trees",
        ...
        action = "store_true",
        default = False,
        help = "Export the gene trees used as input to ASTRAL/ASTER")
    parser.add_argument('-a', "--export-all",
        dest = "export_all",
        action = "store_true",
        default = False,
        help = "Export using all options sorted in to separate directories")
...
def check_parameters(parms):
    ...
    if not os.path.isfile(logs_file):
        ...
        sys.exit(1)
    if all(not i for i in [parms.best_ml_tree, parms.best_ml_model, parms.bootstrap_trees, parms.support_value_tree, parms.species_tree, parms.gene_trees]):
...
if (__name__== "__main__"):
    ...
    if p.export_all:
        ...
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)
        export_best_ml_tree(p.input_dir, out_dir, families)
        out_dir = os.path.join(p.output_dir, "best_ml_model")
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)
        export_best_ml_model(p.input_dir, out_dir, families)
        out_dir = os.path.join(p.output_dir, "bootstrap_trees")
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)
        export_bootstrap_trees(p.input_dir, out_dir, families)
        out_dir = os.path.join(p.output_dir, "support_value_trees")
        if not os.path.exists(out_dir):
            os.makedirs(out_dir)
        export_support_value_trees(p.input_dir, out_dir, families)
